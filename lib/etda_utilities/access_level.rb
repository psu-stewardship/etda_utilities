module EtdaUtilities
  class AccessLevel
    ...
    def self.partner_access_levels
      @partner_access_levels = YAML.load_file('lib/etda_utilities/access_levels_for_partners.yml')[EtdaUtilities::Partner.current.id]
    end
    ...
    private
      ...
      def i18n_handle
        "#{EtdaUtilites::Partner.current.id}.access_level.#{self}"
      end

      def submission_attributes
        x = 'open_access'
        @attributes = x.enumerized_attributes['open_access']
